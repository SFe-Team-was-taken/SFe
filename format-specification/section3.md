...
Orders of chunks in all SF files are strictly defined, as in SF versions 2 and 3, and should be kept to, with the exception of TSC mode.

## 3.1a Trailing sdta Chunk ("TSC") mode

As Saga of OpenMPT has found out, it is in fact possible to increase the maximum size of the 32-bit SF 2.04 format to 8 gibibytes (GiB), if the sdta chunk is placed at the end of the file rather than in the middle as usual for SF 2.04. This will be referred in the SFe32 specification as trailing sdta chunk mode, or TSC mode for short.
...
The 3 main chunks should appear in this order, and the nine sub-chunks of the "pdta" chunk should appear in the order described. The only exception is with TSC mode.
